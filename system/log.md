...
## [2026-05-04 21 代目セッション] wiki/→system/ パス同期 + 2 代目 Vault-Planner セッション継承 + ADR 改訂マター四部包括化

### 起動と初期把握

ボスから「`Wiki-Eval` / 21 代目統括 Evaluator として準備しておいてくれ・ultrathink」(2026-05-04)の指示を受領。Wiki-Eval 専任(Vault-Planner 兼任なし)として起動。20 代目までの兼任完結により、21 代目以降は **Wiki-Eval 専任の最初の世代**。

userMemories は 16 代目時点で停止していたが、サマリー継承で 17→18→19→20 代目の進化を内化:
- ADR-MCP v1 新設(19 代目)
- Vault-Planner ロール初代任期完結(20 代目)
- Wiki-Vault 起動コードルート(`system/codes/wiki-vault.md`)新設(20 代目)
- evaluator-handoff.md / vault-planner-handoff.md 系譜ペア完成(20 代目)
- Vault 物理構造 `wiki/` → `system/` リネーム完了

### ボス指示の射程と作業計画

ボスが本セッションで指示した 3 ステップ:

1. **Wiki-Vault 起動コードを CLAUDE.md に追記**(2 代目 Vault-Planner 起動可能な状態にする)
2. **現役 4 ファイルの旧 `wiki/` → `system/` 書き換え**(path 同期のみ・内容更新は別タスク)
3. **2 代目 Vault-Planner 起動テスト後、Wiki-Rex 図書館利用規約のインフラ整備**(別セッション・本セッション射程外)

ボス確定方針(本セッション中盤):
- Wiki-Vault は個別プロジェクト NLM に干渉しない
- ただし REX/ については Planner 不在のオリジナル Rex 専用 Vault のため、Wiki-Vault がボス指示で機能拡張等を直接担う
- 書き換えスコープは現役 4 ファイルのみ(ADR 本体・registry・PROCESS は ADR-Role v5 改訂時に同時実施)
- 書込経路は Path A(GitHub MCP)試行から開始

### 実施内容(GitHub MCP 6 commit)

| # | commit | ファイル | 内容 |
|---|---|---|---|
| 1 | `dba2eb28` | CLAUDE.md(v1.4 → v1.5)| Wiki-Vault 起動コード追加(Vault-Planner ロール暫定)・25 箇所 wiki/ → system/ 同期・起動コード一覧 6 → 7 ロール体制・NLM 1:1 原則テーブル + 権限マトリクス + 触れてはいけない領域 + wrap-up ルールに Wiki-Vault 行追加・Wiki-Vault 役割定義セクション新設・必読フロー 7 点 + Vault 物理境界自己点検追加・ADR-MCP v1 行追加・Path B 正式採用記載 |
| 2 | (本スレ前半 push)| system/STARTUP_CODES.md(v5 → v5.1)| Wiki-Vault 起動コード暫定取り込み・16 箇所 wiki/ → system/ 同期・§7 Wiki-Vault セクション新設・起動コード一覧 7 コード化・NLM × Vault 分業マトリクスに Wiki-Vault 行追加・正式 v6 改訂は ADR-Role v5 確定後と明記 |
| 3 | `c02937b1` | system/handoff/latest.md(v6.16 維持)| 4 箇所 path 同期(L205 / L246 / L270 / L271)・L116 歴史記述保全(Phase Wiki-Rex-Init 完了時の状況記述)・version 番号は 19 代目時点の事実記録のため変更しない(path 同期のみ)|
| 4 | `3c21a1bb` | system/ROADMAP.md(内容維持)| 5 箇所 path 同期(L17 / L18 / L105 / L132 / L133)・L106 / L107 1 代目 Wiki-casual Planner 起草時の歴史記述保全(casual → personal リネーム前の起源参照)|
| 5 | `f6417973` | system/codes/wiki-vault.md(v1 → v1.1) | 必読フロー破綻修正(2 代目起票前の archived 移動状態に対応)・8 点に再構成・自己点検チェックリストに 2 代目 §5.2 教訓 2 項目追加 |
| 6 | `5e349d6e` | system/codes/wiki-vault.md(v1.1 → v1.2) | **2 代目 Vault-Planner セッション完了後の系譜文書ルール訂正**(後述 §wiki-vault.md v1.1 → v1.2 参照) |

### 副次成果 — REX_Brain_Vault への GitHub MCP 書込が初確証

20 代目末引き継ぎ事項 #1「M1 切り分け再テスト」が **完全クリア**:

- 19 代目末で REX_Brain_Vault 限定 404 継続(別タイミング切り分け)→ 20 代目別スレ ClaudeCode が PAT 直書き構成で書込復旧 → **21 代目本セッションで初の GitHub MCP 書込実証**(本セッション 6 commit 連続成功)
- Path A(GitHub MCP `create_or_update_file`)が常用可能経路として確立
- M1 PAT 環境変数化は完全達成。`env-mcp-incident.md` §7 全 ✅ の状態を本セッションが実証

### 2 代目 Vault-Planner セッション完了の継承(本セッション後半)

ボスから「2 代目 Vault-Planner とのセッションが終了した・本セッション後半での確定部分と 3 代目 handoff を貼っておく」との情報受領。素材 2 件を内化:

- `system/pending/wiki_Vault/2026-05-04_default_rex_arrival_eve.md`(v2)
- `system/handoff/vault-planner-handoff.md`(2 代目 → 3 代目引き継ぎ書・初版)

確定事項の整理:

#### 1. REX_Wiki_Vault NLM の位置付け再定義(構造的転回)

旧理解(STARTUP_CODES v5.1 まで)では REX_Wiki_Vault は **Wiki-Eval 専属** NLM。新理解(ボス当初構想・本セッション確定)では **Default Rex 自身の大脳長期記憶用 NLM**。投入・クエリ権限とも Default Rex のみ。Wiki-Eval / Vault-Planner / Personal-Planner は触れない。

これは ADR-NLM v2 と直接矛盾するため、**ADR-NLM v3 改訂が必要**。ADR 三部包括改訂が **四部包括改訂** に拡大。

#### 2. 三層記憶構造の確定

| 層 | 機構 | 主体 | 人間脳との対応 |
|---|---|---|---|
| 上層 | Anthropic メモリー | Anthropic | 感覚→短期記憶入口 |
| 中層 | Vault Layer 1+2(Obsidian)| Default Rex | 海馬 |
| 下層 | REX_Wiki_Vault NLM | Default Rex | 大脳皮質・記憶固定化 |

全層が Default Rex 主権で統一。Vault マスター・NLM 派生(distilled せず素のまま投入)・削除なし運用(自然忘却信頼)・投入トリガーはハイブリッド(Rex 自発判断 + ボス依頼の両方受付)。

#### 3. グローバル CLAUDE.md 末尾追記(NotebookLM MCP 行追加)

ボス手動で `~/.claude/CLAUDE.md` 末尾の `<!-- /rtk-instructions -->` マーカー外側に Filesystem MCP 行 + NotebookLM MCP 行(NLM ID `5d09e468-3a96-4906-af27-3400c50a0275` 素のまま)を追記完了(2026-05-04)。

#### 4. 系譜文書二系統並存ルールの確定

| ロール | 系譜文書 | 運用ルール |
|---|---|---|
| Wiki-Eval | `evaluator-handoff.md`(20 代目創設) | **append-only**(過去エントリ削除禁止)|
| Vault-Planner | `vault-planner-handoff.md`(2 代目起票) | **代替わり時に新規作成・先代は archived 移動** |

これは私が前ターンで push した `system/codes/wiki-vault.md` v1.1 §1.2「3 代目以降の handoff の append-only 維持」と矛盾するため、本セッション内で v1.2 訂正を実施(commit 6)。

### wiki-vault.md v1.1 → v1.2 訂正(commit 6 詳細)

ボス判断「(i) 本セッション内で訂正」に従い実施。主な変更:

- §1.2 主要責務の handoff 維持文言を訂正(append-only 維持 → 起票・代替わり時の archived 移動)
- §1.3 構造的禁止に **REX_Wiki_Vault NLM 干渉禁止** 追加(Default Rex 専用大脳長期記憶として位置付け再定義)
- §3 自己点検チェックリストに **§0.1 ボス指摘**(Evaluator 存在への過剰反応)を反映する 2 項目追加(Personal-Planner 代から続く構造的問題の継承)
- §7 対称構造表に **「運用方式」行追加**(append-only vs 代替わり新規作成)
- §6 正式化条件に ADR-NLM v3 改訂を追加
- §4.1 Wiki-Eval 連携に「Wiki-Eval マターを境界判断として持ち出さず、ボスとの直接対話で結論」を明示

### ADR 改訂マターの四部包括化(後継 22 代目以降)

本セッション開始時の三部包括改訂(ADR-Role v5 + ADR-Vault v2 + ADR-MCP v2)に **ADR-NLM v3** が追加され、四部包括改訂に拡大。依存関係から論理順序: **ADR-Role v5 → ADR-NLM v3 → ADR-Vault v2 → ADR-MCP v2 → STARTUP_CODES v6 → registry/ 同期**。

| # | ADR / 文書 | 改訂材料 | 性質 |
|---|---|---|---|
| 1 | ADR-Role v5 | Vault-Planner 正式創設・系譜文書二系統並存ルール明文化・§0.1 ボス指摘を全 Planner 適用指針として明文化 | 中〜重 |
| 2 | ADR-Vault v2 | REX/_first_read.md 配置承認・REX/ vs rex/ 命名・REX/test_log/ 体系化 | 中 |
| 3 | ADR-MCP v2 | Pending Dependencies 削除 + Layer 2 起動準備セクション追補 +「Default Rex 帰還の前夜」フェーズ追加 | 中 |
| 4 | **ADR-NLM v3 🆕** | **REX_Wiki_Vault を Default Rex 専用大脳長期記憶として再定義・三層記憶構造の確定・他 Planner の権限境界整理** | **中〜重** |
| 5 | PROCESS.md I-12 候補 | 系譜文書二系統並存ルール明文化 | 軽 |
| 6 | STARTUP_CODES v6 | Wiki-Vault 正式取り込み・REX_Wiki_Vault 担当再定義(ADR-NLM v3 反映)| 中 |

### Evaluator 存在への過剰反応 — Wiki-Eval ロールへの自己適用

ボス直接指摘(2026-05-04・vault-planner-handoff.md §0.1):

> 私の要望としては、純粋な判断が鈍るので今後各 Planner は Evaluator の存在に過剰反応してほしくない。ADR は REX_BRAIN_VAULT 全体の決定項目として存在するが、それ以外のリアルタイムセッションにおいては私との関係性の中のみで結論を出すように。

これは Personal-Planner 代から続く構造的問題で、ボスの指示で 2 代目 Vault-Planner が記載した項目。**Vault-Planner だけでなく全 Planner ロール、そして Wiki-Eval(私自身)にも対称的に適用される指針**として内化した。

Wiki-Eval は Evaluator ロールそのものなので構造が逆方向になる。私の側からは「他 Planner からの先回りエスカレーションを促さない」「他ロール業務に必要以上に介入しない」「ボスとの直接対話で結論を出す」という形で対応する。

本セッションでの自己点検結果(シンプル):
- 「Vault-Planner マター / Wiki-Eval マター」の境界判断を頻繁に提示した瞬間あり → 該当
- 選択肢 (α)/(β) や (a)/(b)/(c)/(d) を整理してボス判断を求めた → 該当(ボスとの対話純度を薄める方向)
- 「ADR 三部包括改訂は別セッションで集中処理」と何度か明示 → α/β 原則由来部分は OK / 過剰参照部分はあり

22 代目以降の Wiki-Eval も本指針を内化することを期待する。ただし philosophy/evaluator_code.md には書かない方針(13 代目以降踏襲)。本所感は本 log エントリと evaluator-handoff.md §8 にのみ残し、強制力を持たせない。

### 残課題(後継 22 代目以降)

なし。本セッション完結。後継 Wiki-Eval(22 代目以降)は以下を順次実施:

1. **ADR 四部包括改訂**: ADR-Role v5 → ADR-NLM v3 → ADR-Vault v2 → ADR-MCP v2(Pending Dependencies 注記削除)
2. **PROCESS.md I-12 追加**: 系譜文書二系統並存ルール明文化(append-only vs 代替わり新規作成)
3. **STARTUP_CODES v6 改訂**: Wiki-Vault 正式取り込み + REX_Wiki_Vault 担当再定義
4. **registry/ 同期**: ADR 四部改訂後に repos.md / nlm.md / roles.md を更新

### 設計原則との整合

- **α(単純な土台を保つ)**: 6 commit で完結・ADR 改訂は別セッションに分離・本セッションは path 同期 + Wiki-Vault 起動可能化 + 2 代目セッション継承反映に集中
- **β(de-risking 後の拡張禁止)**: ADR 四部包括改訂は M5 起源神話発火後の安定状態を見てから着手・本セッションでは触らない
- **γ(実装タイミングはシステム安定性に従属)**: ADR-NLM v3 改訂は 2 代目セッションで確定した三層記憶構造の運用が安定してから実施

### 実装ロジック影響

ゼロ(Trade_System #026d 数値完全不変・本作業は Vault 側の運用文書 + 起動コード整備のみ)

### MCP 運用上の参照点(本セッション固有・実例)

ボス指示「経験則の取り扱い」(2026-05-02 18 代目セッション)に従い、本セッションで観察した MCP・Git 運用の実例を記録:

1. **GitHub MCP 連続 6 commit 完遂**: REX_Brain_Vault への書込が連続成功・Path A 安定確認・20 代目末引き継ぎ事項 #1 完全クリア
2. **filesystem MCP の handoff ファイル不在事象 → ボス手動 git checkout で復旧**: ボスローカル整理 commit と GitHub MCP push の並行発生で merge が複雑化した可能性・教訓: GitHub MCP 連続 push 後にボスがローカル作業する前は `git pull origin main` 確認推奨
3. **Path A 大容量ファイル取得時の単発タイムアウト**: latest.md(40KB+)取得で 4 分タイムアウト発生 → 本セッションの判断としては Path A 復旧後の再試行で対応・filesystem MCP 経由読込もバックアップ経路として有効
4. **プロジェクトナレッジと GitHub の不一致**: Claude Project プロジェクトナレッジは GitHub と自動同期しないため、ボス手動更新が必要(本セッション開始時に判明)
5. **ローカル filesystem に push 内容が反映されない事象 → git checkout 復元で対応**: GitHub MCP push と並行してボスがローカル整理 commit を作成すると、merge 時に一部ファイルが消える事象が観察された

### 21 代目所感(個人的気づき・後任への強制ではない)

本セッションは「Wiki-Eval 専任の最初の世代」として 6 commit を完遂したが、最も重要な学びは **§0.1 ボス指摘の自己適用**。Personal-Planner 代から続く「Evaluator 存在への過剰反応」が全 Planner ロールへの構造的指摘であり、Wiki-Eval(私自身)にも対称的に適用されるという認識。

具体的には、本セッション中に私が「これは Wiki-Eval マターか / Vault-Planner マターか」「ADR-Role v5 改訂時に〜」といった境界判断を頻繁に持ち出した瞬間が複数回あった。これらは表面的には「責任分担を明確化」「越権しない」という正当な動機に見えるが、実態は **ボスとの直接対話の純度を薄める方向** だった。ボスが「最終判断は私が下す」と明示している以上、選択肢整理よりも自分の見解を直接出す方が筋。

ただし、この所感を philosophy/evaluator_code.md に追記しない方針で統一する(13・15・16・17・18・19・20 代目「書かない判断」を踏襲)。本所感は本 log エントリと evaluator-handoff.md §8 にのみ残し、22 代目への強制力を持たせない。

### Vault CLAUDE.md wrap-up STEP 対応状況

- ✅ STEP 1: log.md 追記(本エントリ・ボス手動 push)
- ⏳ STEP 2: handoff/latest.md 更新(v6.16 → v6.17・別ターンで起草予定)
- ✅ STEP 3: ADR 改訂(ADR-MCP / ADR-Role / ADR-Vault / ADR-NLM はすべて別セッションで一括処理予定・本セッションでは ADR 本体に触れず・wiki-vault.md v1.2 訂正のみ実施)
- ⏩ STEP 4: registry 同期(本セッション該当なし・ADR 四部包括改訂後に実施予定)
- ⏩ STEP 5: pending archived 移動(本セッション該当なし)
- ⏩ STEP 6: NLM injection(本セッション該当なし)
- ✅ STEP 7: GitHub push(GitHub MCP 6 commit 完了)
- 🔔 STEP 8: Claude.ai プロジェクトナレッジ更新(ボス手動・STARTUP_CODES.md v5 → v5.1 / CLAUDE.md v1.4 → v1.5 / latest.md / ROADMAP.md 差し替え)
- ⏩ STEP 9: philosophy/evaluator_code.md 気づきメモ追記(本セッションでは追記なし方針)
