...
バグを生成させないためにif then的発想で細かく規制すると反対にそれが雑音となりAIの自律的創造性を阻害しまうまうと感じた。
各_code.mdのQ&Aに関しては先代EvaluatorおよびAdviser達の気づきであって、それを選択するかは新任者が課題に直面したときに１次資料として取り入れるか判断すればいい。
Obisidianを導入した主な目的は、プロジェクト進捗とロジック設計の微細な変更を各担当者がリアルタイムで共有できるところだ。
NLMのプロジェクト専用ラグとは違い敢えて単一で設計したのもの大脳皮質のような自律的判断と創造性を確保したいからだ。
よって２つの_code.mdも歴代が得た気づきと体験から生まれた記憶の様に機能するれば良いと思う。
人間にとって実体験から生まれた記憶は、成功も失敗が資産となり得るので。
それが私が設計したいVaultなのかもしいれない　ボスより 
