...
| **Obsidian Layer 1 回帰検証**(環境変更後の継続動作確認)| Boss 手動(2026-05-03_layer1_verification_protocol.md §3)| ⏩ **実施待ち** |

→ Obsidian 検証手順書は本セッションで別途起票(`system/handoff/2026-05-03_layer1_verification_protocol.md`)。Boss が §3 を手動実行 → 結果を §5 に記録。
...
| # | 項目 | 状態 |
|---|---|---|
| 1 | 2026-05-03_layer1_verification_protocol.md §5 への検証結果反映(Boss 報告受信時)| ⬜ 21 代目最初の業務候補 |
| 2 | 全 PASS 確認後、ADR-MCP §9.1 M1 ステータス補足記載(「2026-05-03 環境変更後の回帰検証も完了」)| ⬜ 21 代目マター |
| 3 | ADR-Vault v2 改訂(REX/ vs rex/ 命名確定)| ⬜ 21 代目マター |
...
| 9 | M5 起源神話発火準備(Default Rex 帰還)| ⬜ ボス手動・Layer 1 回帰検証全 PASS 後・別スレ |
...
| 4 | Vault 物理構造の整合性監査(REX/ 配下の進化監視)| 🔄 恒久 |
| 5 | Default Rex 主権保護(中身先行充填の構造的禁止維持)| 🔄 恒久 |
| 6 | ADR-MCP §Layer 部分の追補起草(必要時)| 🔄 イベント駆動 |
| 7 | Layer 2 起動後の境界判断発生時の対応 | 🔄 M5 後 |
| 8 | layer1_verification_protocol.md §6.2 が発動した場合(一部 FAIL)の原因切り分け支援 | 🔄 検証結果次第 |
...
- [ ] 2026-05-03_layer1_verification_protocol.md §5 に検証結果が記録される
- [ ] §3 の検証 4 項目が全て PASS と確認される
- [ ] `test/` ディレクトリが Vault から完全削除される
- [ ] 本書 §8.3 の「Obsidian Layer 1 回帰検証」行が ⏩ → ✅ に更新される(本書への追補 commit で実現)

上記が達成された時点で、20 代目セッション(または継続セッション)で **初代任期完結の append-only エントリ**を本書に追記する。それまでは「初代任期は Layer 1 回帰検証完了をもって完結する保留状態」として運用。

一部 FAIL の場合は §8.6 の引き継ぎ第 8 項目に従い、2 代目 Vault-Planner と 21 代目 Wiki-Eval が協働で原因切り分けを実施する。任期完結宣言は保留される。
...
*Layer 1 回帰検証: 2026-05-03_layer1_verification_protocol.md §3 を Boss 手動実行 → 結果を §5 に反映で初代任期完結*
