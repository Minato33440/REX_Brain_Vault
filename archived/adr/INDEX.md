...
最終更新: 2026-05-02(19代目統括Evaluator・ADR-MCP v1 新設反映 + Vault-Planner 仮設定線維持)
...
| **[ADR-MCP](ADR-MCP.md)** | **REX_AI Vault Memory Connection Architecture** | **Accepted (with pending dependencies)** | **2026-05-02** | **v1** | **-** |
...
> **ADR-MCP v1 の Pending Dependencies**: ADR-Vault v2 / ADR-Role v5 改訂が完了するまで「Pending Dependencies 状態」として扱われる。三部包括改訂完了後、本注記は ADR-MCP 内部から削除可能。
...
## 新設 ADR(supersede ではなく新規)

| ADR | 制定日 | 起草者 | 主な内容 |
|---|---|---|---|
| [ADR-MCP v1](ADR-MCP.md) | **2026-05-02** | **19 代目統括 Evaluator(Vault-Planner 暫定兼任・仮設定中)** | **Phase Two-Vault-Init 統合 ADR / Layer 1 + Layer 2 採用経路(filesystem MCP / Path X)+ M2/M3 defer + Origin Myth 新定義 + Vault-Planner 仮設定線 + Pending Dependencies 注記** |
...
ADR-Role v4 (基盤: 全ADRの権限定義 + 1:1 NLM原則 + 二系統管轄 + 6ロール体制 + Wiki-Rex 読み取り専用クエリ)
  ...
  │           └── ADR-MCP v1 (Vault Memory Connection Architecture / Layer 1 + Layer 2)
  └── ADR-NLM (NLM構造 + 担当ロール 1:1)
```

セッション開始時の読込推奨順: ADR-Role → ADR-Repo → ADR-Vault → ADR-NLM → ADR-MCP

> **Pending Dependencies**: ADR-MCP v1 は ADR-Vault v2 / ADR-Role v5 改訂を前提とする。三部包括改訂完了まで本依存関係図も暫定。
...
- **新規 ADR 起草の例外**: Phase 関連の統合 ADR(例: ADR-MCP v1)は複数 pending を統合する形で新設可能(本 INDEX で Supersede 履歴とは別の「新設 ADR」セクションに記録)
...
- **本体ファイル名は常に固定**(`ADR-Role.md` / `ADR-NLM.md` / `ADR-MCP.md` 等・日付やバージョンを含めない)
