> **[SUPERSEDED by ADR-NLM v2 (2026-04-28)]**
> 本ファイルは ADR-NLM v1（2026-04-27 制定）のアーカイブ版です。最新の確定事項は `wiki/adr/ADR-NLM.md` を参照してください。
> Supersede 経緯: 14 代目統括 Evaluator により REX_Casual_Brain → REX_Personal_Brain 改名（UUID 不変）が ADR 化され、本 v1 はその対象として置換されました。
