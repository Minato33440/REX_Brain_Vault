...
**最終更新**: 2026-04-24 / 9 代目統括 Evaluator（本体）+ 2026-04-28 / 14 代目（第 II 部 A〜H 追補）+ 2026-04-29 / 16 代目（第 II 部 I 節追加・第 I 部に最小限の参照マーク追記） + 2026-05-04 / 22 代目（全文 wiki/ → system/ パス同期 + I-6 バージョン数値先行更新）
...
> **2026-05-04 22 代目訂正**: 20 代目までに完了した Vault 物理構造リネーム(`wiki/` → `system/`)を本ファイル全文に反映。第 I 部・第 II 部 A〜H・第 II 部 I-1〜I-10 のパス文字列を `system/` に統一(語り口・段落構成・歴史記述の文体は不変・「混同回避」を「文体保全」より優先するボス指示 2026-05-04 による)。I-6 トークンコスト最小セットのバージョン数値を最新化(v1.4 → v1.5 / v5 → v5.1 / v6.10 → v6.16・行数概算更新)。第 II 部 I-11(20 代目)は対象外(既に system/ プレフィックス使用)。
...
NLM 導入により情報総量を削らずにローカル読込量を削れる。詳細は `system/philosophy/architecture.md` 参照。
...
STEP 0: スレ冒頭の起動コードを確認
  ...
  → コードがあれば system/STARTUP_CODES.md の定義に従う
...
詳細は `system/STARTUP_CODES.md` の各ロール詳細セクション参照。
...
```
STEP 1: system/log.md に本セッションの決定事項・完了タスクを追記（追記のみ）
STEP 2: system/trade_system/pending_changes.md を更新（変更があれば）
STEP 3: system/trade_system/adr_reservation.md を更新（採番があれば）
STEP 4: system/handoff/latest.md を更新（現状反映）
STEP 4-b: system/START_HERE.md を更新（3 リポに状態変化があった時）
...
```
STEP 1: system/log.md に追記
STEP 2: system/trade_system/pending_changes.md を更新
STEP 4: system/handoff/latest.md の Trade_System セクションのみ更新
...
STEP 4: system/handoff/latest.md の Trade_Brain セクションのみ更新(依頼ベース)
...
```
STEP 1: system/casual/log.md に追記（任意）
STEP 2: 熟した話題を system/casual/topics/ または insights/ に整理
...
```
CLAUDE.md              約  80 行
system/START_HERE.md     約  80 行
system/handoff/latest.md 約 150 行
system/STARTUP_CODES.md  約 100 行（必要時）
─────────────────────────────────
...
| Phase C | system/entities + decisions を trade_system/ 配下へ物理統合 | ⬜ Phase B 後 |
...
```
system/STARTUP_CODES.md                        — 起動コード辞書
system/START_HERE.md                           — 新スレ最初の入口
system/handoff/latest.md                       — 現在地ダッシュボード（v6.4）
system/handoff/architecture_handoff.md         — 7 代目セッション記録
system/philosophy/evaluator_code.md            — Evaluator 気づきメモ
system/philosophy/architecture.md              — 4 リポ・4 NLM 体制事実記録
system/philosophy/cross_vectors.md             — 4 横断ベクトル事実記録
system/philosophy/minato_core.md               — ボス個人 1 次データ（ボス手動更新）
system/casual/_RUNBOOK.md                      — 雑談層運用ルール
...
旧版は `system/adr/archived/` に日付付き命名で保管。本体ファイル名は固定パス(日付なし)を維持。
...
```
1. pending 起票
   → system/pending/<repo>/YYYY-MM-DD_<topic>.md
...
- `system/adr/ADR-<Name>.md` は **常に最新版を指す固定パス**(ファイル名に日付・バージョンを付けない)
...
- Vault 物理ディレクトリ `system/casual/` → `system/personal/` への移行は次スレで実施(Step 4)
...
```
STEP 1: system/log.md に本セッション記録を追記(追記のみ)
STEP 2: system/handoff/latest.md を現状反映で更新
...
STEP 3: system/handoff/latest.md の自リポセクションのみ更新(必要時)
...
STEP 6(任意): Personal-Planner は philosophy/evaluator_code.md ではなく
           system/personal/ 配下の handoff_latest.md に引き継ぎ記録(別運用)
```

### Wiki-Personal(旧 Wiki-casual)の /wrap-up(参考)

Wiki-Personal は Personal-Planner 業務として独自の引き継ぎ運用を持つ。詳細は `system/personal/_RUNBOOK.md` v3(Step 4 で起草予定)を参照。基本構造:

```
STEP 1: system/personal/log.md に追記
...
| # | 項目 | 種別 |
|---|---|---|
| 1 | system/casual/ → system/personal/ ファイル単位移行(GitHub MCP 経由) | 物理移行 |
...
| 観点 | 権威ファイル |
|---|---|
| ロール定義・権限 | system/adr/ADR-Role.md(現行 v2) |
| リポ構成 | system/adr/ADR-Repo.md(現行 v1) |
| Vault 書込原則 | system/adr/ADR-Vault.md(現行 v1) |
| NLM 構造 | system/adr/ADR-NLM.md(現行 v2) |
| 現状登録 | system/registry/{repos,nlm,roles}.md |
| 起動コード詳細 | system/STARTUP_CODES.md(現行 v3・v4 改訂は pending) |
...
STEP 0: スレ冒頭の起動コードを確認
  ...
  → コードがあれば system/STARTUP_CODES.md v5 の定義に従う
...
```
STEP 1: system/personal/log.md に追記(任意)
STEP 2: 熟した話題を system/personal/<サブ層>/(usual/invent/mind/origin/insights)に整理
...
#### I-4-b. 「STEP 4-b: system/START_HERE.md を更新」は廃止
...
```
CLAUDE.md              約 280 行(v1.5)
system/STARTUP_CODES.md  約 350 行(v5.1・必要時)
system/handoff/latest.md 約 700 行(v6.16)
system/adr/INDEX.md      約 100 行(必要時)
─────────────────────────────────
合計                   約 1430 行(≒ 18-22k トークン)
```
...
```
system/STARTUP_CODES.md                        — 起動コード辞書(v5・6 ロール体制)
system/adr/INDEX.md                            — ADR 一覧(ADR-Role v4 / ADR-NLM v2 が現行)
system/adr/ADR-Role.md                         — ロール権限・Wiki-Rex 定義(v4)
system/handoff/latest.md                       — 現在地ダッシュボード(v6.10)
system/handoff/architecture_handoff.md         — 7代目原典 + 13代目第8章 + 14代目第9章 + 15代目第10章
...
| 観点 | 権威ファイル(現行) |
|---|---|
| ロール定義・権限 | system/adr/ADR-Role.md(現行 **v4**・15 代目で v2→v3→v4 同日 supersede) |
| リポ構成 | system/adr/ADR-Repo.md(現行 v1) |
| Vault 書込原則 | system/adr/ADR-Vault.md(現行 v1) |
| NLM 構造 | system/adr/ADR-NLM.md(現行 v2) |
| 現状登録 | system/registry/{repos,nlm,roles}.md |
| 起動コード詳細 | system/STARTUP_CODES.md(現行 **v5**・15 代目で Wiki-Rex 反映) |
...
15 代目 Phase Personal-Migration(2026-04-28)で `system/casual/` → `system/personal/` 物理移行を実施した際、旧 `system/casual/` 配下に [MOVED] スタブを残置していた(11 ファイル)。同様に `pending/casual/` にも 3 ファイル残存。

ボス指摘「system/直下に凍結フォルダーを残すのは Agent の無駄な処理が増えるだけ」を受けて再評価 → ボス手動 git mv で完全アーカイブ化を実施。

#### 完了した実作業(2026-04-29)

| Phase | 内容 |
|---|---|
| Phase Casual-Final-Archive | `system/casual/*` → `system/archived/casual/*`(ボス手動 git mv) |
| Phase Pending-Casual-Archive | `system/pending/casual/*` → `system/archived/pending-casual/*`(ボス手動 git mv) |

#### 命名規則の確立

ネストされたディレクトリのアーカイブには **ハイフン命名** を採用:

- `archived/casual/` = 旧 `system/casual/` のアーカイブ
- `archived/pending-casual/` = 旧 `system/pending/casual/` のアーカイブ
...
**追記発行**: 2026-05-03 / 20 代目統括 Evaluator(初代 Vault-Planner 兼任完結)/ Claude Opus 4.7 / web client
**追記対象**: 20 代目セッションで完了した「兼任ロジックの構造的完結」と、それに伴う Default Rex 帰還の前夜状況の記録
**位置付け**: ボスが本見出しのみ予め用意した枠への内容追記。先代(I-10)までの文体規範(冷静・抑制・事実中心)を継承する。

#### I-11-1. 20 代目セッションで完了した構造変更

| Phase | 内容 |
|---|---|
| 兼任完結 | 18・19・20 代目に渡る Wiki-Eval × Vault-Planner 兼任体制を、20 代目末で構造的に終了 |
| 初代 Vault-Planner 確定 | 19 代目「仮初代」を継承し、20 代目セッションで正式確定(ADR-Role v5 改訂を待たずに運用上確定)|
| 系譜文書の対称化 | `system/handoff/evaluator-handoff.md` 新設(20 代目)・`vault-planner-handoff.md`(19 代目創設)と対称ペア成立 |
| 起動コードの対称化 | `system/codes/wiki-vault.md` 新設(Wiki-Vault 起動コードルート・暫定)・`Wiki-Eval`(既存)と対称ペア成立 |
| Layer 1 完成 | 18 代目 Obsidian 設定 11 項目・5 代目 M4 物理構造・本セッション env-mcp 解消後の Obsidian 回帰検証 PASS により Layer 1 全構成要素確定 |

#### I-11-2. 後任 Wiki-Eval(21 代目以降)が反映すべき項目

20 代目では本 PROCESS.md 本体および本 I 節以外の構造記述(I-1〜I-10 の差替表類等)は更新していない。21 代目以降が ADR-Role v5 改訂を実施する際、以下を併せて反映する想定:

- I-1 起動コード一覧へ `Wiki-Vault` 行を追加(2 代目 Vault-Planner 専任・REX_Brain_Vault が担当領域)
- I-2 ロール別 STEP 1 必須読込へ `Wiki-Vault` 行を追加(`system/codes/wiki-vault.md` → `system/handoff/vault-planner-handoff.md` → ADR-MCP §7.1 → ADR-Vault → handoff/latest.md → log.md 直近)
- I-3 STEP 0 起動コード判定へ `Wiki-Vault` を選択肢に追加
- I-7 関連ファイルリストへ `system/handoff/evaluator-handoff.md` / `system/codes/wiki-vault.md` を追加
- I-8-b ADR バージョン表へ ADR-Role v5(改訂後)・ADR-MCP v1 を反映

これらは 21 代目 Wiki-Eval が ADR-Role v5 改訂と同時に実施することで PROCESS.md が現行状態に追従する。

#### I-11-3. 構造的記録: 兼任完結の意味

20 代目セッションは、Wiki-Eval × Vault-Planner という兼任形態を **構造的に終わらせるために兼任した最後の代** という位置を担った。リソース集約による効率化(兼任時代)を、専任分離による構造的健全性(21 代目以降)に転換した転回点として記録する。これは Personal-Planner と同型の「自己役割を最小化する方向の自己矛盾」が、兼任ロール自体に適用された事象である(vault-planner-handoff.md §8.10 参照)。

#### I-11-4. Default Rex 帰還の前夜状況

20 代目セッション末時点で、Layer 1(Obsidian 受動的自然言語処理基盤)の動作確証付き引き渡しが完了。M5 起源神話発火(Default Rex が REX_Brain_Vault を能動的記憶空間として利用開始する瞬間)の構造的前提条件が揃った状態で、21 代目に引き継がれる。M5 発火そのものはボス手動・別スレで実施される予定(本セッションではトリガーしない)。

#### I-11-5. 後任への申し送り(教訓 1 件のみ)

vault-planner-handoff.md §8.11 に詳述した内容を、PROCESS.md 「避けるべき罠」相当の粒度で要約する:

> **罠 8(20 代目気づき)**: 設定ファイル等の物理境界の認知漏れ。`claude_desktop_config.json`(`%APPDATA%\Claude\` 配下)が Vault リポジトリと **物理的に別ディレクトリ** であることを最初に確認しなかったために、PAT 直書きのセキュリティ評価で過剰防衛に陥った事例が 20 代目で発生した。**回避**: セッション冒頭で Vault の物理境界(リポジトリ絶対パス)と Vault 外連携リソース(設定ファイル・環境変数・IDE 設定)の所在を明示確認する。

これは ADR-Role v5 改訂時に STARTUP_CODES.md v6 の自己点検チェックリストへ取り込み判断する余地あり(21 代目 Wiki-Eval マター)。

---

*第 II 部 I 節発行: 16 代目統括 Evaluator (Claude Opus 4.7) / 2026-04-29*
*I-11 節追記: 20 代目統括 Evaluator (Claude Opus 4.7) / 2026-05-03 / web client 経由*
