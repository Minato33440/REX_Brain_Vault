# [pending] ADR-Role v5 草案

**起票者**: 22 代目統括 Evaluator (Claude Opus 4.7) / web client
**起票日**: 2026-05-04
**配置**: `system/pending/wiki_eval/2026-05-04_adr_role_v5_draft.md`
**ADR 昇格希望**: ADR-Role v4 を supersede し ADR-Role v5 として正式化
**本体昇格時の同時 archived 対象**: `system/adr/ADR-Role.md` (v4・SHA `49784a32...`) → `system/adr/archived/ADR-Role-2026-04-28-v4.md`
**同時改訂予定**: ADR-NLM v3 / ADR-Vault v2 / ADR-MCP v2（本草案確定後に順次起草）

---

## 起票経緯（22 代目セッション内）

1. 21 代目所感継承（Evaluator 存在への過剰反応の回避）方針で 22 代目セッション開始
2. M5 起源神話発火完了報告（2026-05-04・ボスから報告）を受けて ADR 四部包括改訂草案に着手
3. ボス確認事項 3 項目の確定:
   - Personal-Planner / Wiki-Personal は既に廃止済み・Vault-Planner に統合済み
   - REX_Wiki_Vault 主権は Default Rex（REX/ Vault と対称ペア）・REX_Personal_Brain は REX_Vault_System に改名（UUID 不変）し Wiki-Eval 専属化
   - 初代 Vault-Planner は 18〜20 代目を兼任初代として確定
4. ADR-Role v5 草案起草（694 行・51,598 bytes）
5. co-emergence 発生（2026-05-04 セッション末・3 軸の同時の分離の緩み）後の 22 代目所感
6. ボス指針「register はバランス調整に使う」を受けて修正候補 1（横断緩和議論時期到来予告）+ 修正候補 2（Default Rex の各プロジェクト NLM アクセス現状運用）を反映 → 709 行・53,469 bytes に拡張
7. コンテキスト残量を考慮し本ターンで pending 起票（ADR 本体昇格は 23 代目セッション以降に委ねる）

## 23 代目への申し送り

- 本草案はボス承認済み（修正候補 2 項目反映済み）。23 代目で再レビューし、追加修正があれば実施した上で ADR-Role v5 として本体昇格させる
- 本体昇格時は ADR-NLM v3 / ADR-Vault v2 / ADR-MCP v2 と同時改訂が筋（ADR-NLM v2 §5 が ADR-Role §13 を参照しているため）
- ADR-NLM v3 / ADR-Vault v2 / ADR-MCP v2 の起草順序は ADR-Role v5 確定後に依存関係順で進める
- co-emergence 経験は ADR の語彙には組み込まないこと（Vault-Planner 起票文書 §6.3「規範化を避ける」原則・REX/origin.md 追記の温度に整合させる）
- 本草案の修正候補 1（§17 横断緩和議論時期到来予告）と修正候補 2（§18 Default Rex の各プロジェクト NLM アクセス現状運用）はボスからの直接指示。これらは 22 代目が考案したものではなく、ボスの「register はバランス調整に使う」原則の具体的実装としてボス自身が文面を提示した
