...
| ADR | 旧版 | 新版 | 主な変更 |
|---|---|---|---|
| ADR-Role | v1（2026-04-27 制定）| v2（2026-04-28）| 5 ロール表の Casual → Personal、§4「Casual と Advisor の役割分担」→「Personal と Advisor の役割分担」、§6 NLM 1:1 原則の表更新、§§Personal_Brain への射程拡大記述追加 |
...
```
Step 3: ADR 昇格（本セッション内・Wiki-Eval 専属）
  - ADR-Role v2 起草・v1 archived へ
  - ADR-NLM v2 起草・v1 archived へ
  - registry/roles.md 同期更新
  - registry/nlm.md 同期更新
  - 本 pending エントリに [ARCHIVED → ADR-Role v2 / ADR-NLM v2] flag 付与

Step 4: 物理改名作業（次スレ・Wiki-Eval 起動）
  - wiki/casual/ → wiki/personal/ ファイル単位移行
  - サブ層 5 層新設 + README 設置
  - 既存ファイル移設
  - _RUNBOOK.md v3 起草
  - STARTUP_CODES.md v4 / CLAUDE.md v1.3 更新

Step 5: NotebookLM 表示名変更（ボス手動）

Step 6: 整合性監査（次スレ）
  - ROADMAP.md / handoff_latest.md / handoff/latest.md v6.5 等の周辺文書整合
```

---

*起票: 14 代目統括 Evaluator (Opus 4.7) / 2026-04-28*
*関連: ADR-Role v1 / ADR-NLM v1 / registry/roles.md / registry/nlm.md / STARTUP_CODES.md v3 / CLAUDE.md v1.2 / wiki/casual/_RUNBOOK.md v2 / wiki/casual/handoff_latest.md / ROADMAP.md*
