...
最終更新: 2026-05-01(v6.14: 17 代目セッション 2 回目・Two-Vault 物理分離起票・旧 ADR-MCP 草案を Phase 0 議論記録として再分類)
...
## 各ロールの記録先(v6.9 で Wiki-casual → Wiki-Personal 訂正・v6.11 で wiki_eval/ 追加・v6.14 で Personal-Planner 廃止予定明記)
...
> **`pending/wiki_eval/` ディレクトリ Note(v6.11 新設・v6.12 用途拡張・v6.14 で再分類規則追記)**: Wiki-Eval 自身が構造変更案件以外の気づきや §候補を起票する場として 16 代目セッション(2026-04-29)で新設。**v6.12 で用途を拡張**: ADR 草案を pending として保留し後任 Wiki-Eval に引き継ぐ用途も追加(2026-04-30 ADR-MCP 草案が初例)。**v6.14 で再分類規則追加**: 後継草案が登場した場合、旧草案は archived 移動ではなく「Phase 0 議論記録」として保留し、冒頭 ⚠️ ポインタと末尾再分類 Note を追加する形で残す(2026-05-01 旧 ADR-MCP 草案が初例・新草案 Two-Vault 再設計と並列で参照)。
