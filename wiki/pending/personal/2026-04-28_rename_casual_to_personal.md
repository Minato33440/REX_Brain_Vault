...
**影響範囲**: ADR-Role(supersede) / ADR-NLM(supersede) / registry/roles.md / registry/nlm.md / STARTUP_CODES.md(v3→v4→v5) / CLAUDE.md(v1.2→v1.3→v1.4) / wiki/casual/ → wiki/personal/(物理リネーム + サブ層 5 層新設) → wiki/archived/casual/(完全アーカイブ) / pending/casual/ → pending/personal/(物理リネーム) / NotebookLM 表示名(UUID 不変) / handoff/latest.md(v6.5→v6.9 段階改訂)

---

## ステータス（最終更新: 2026-04-29 / 15代目 Wiki-Eval）

- ✅ ADR-Role v2 昇格完了（14代目で実施）
- ✅ ADR-Role v3 supersede 完了（15代目・2026-04-28・commit `aecf7f1`）
- ✅ **ADR-Role v4 supersede 完了（15代目・2026-04-28・commit `19943cc` / Phase Wiki-Rex-Init）**
- ✅ ADR-NLM v2 supersede 完了（14代目で実施）
- ✅ Phase Personal-Migration 物理移行 Step 1〜4 完了（15代目・2026-04-28・commit `e07a164`）
- ✅ STARTUP_CODES v4→v5 / CLAUDE.md v1.3→v1.4 / latest.md v6.7→v6.8 改訂完了（15代目・2026-04-28）
- ✅ NotebookLM 表示名変更（REX_Casual_Brain → REX_Personal_Brain）完了（ボス手動・15代目セッション中報告受領）
- ✅ **wiki/casual/ → wiki/archived/casual/ ボス手動 git mv 完全アーカイブ完了（Phase Casual-Final-Archive・2026-04-29）**
  - [MOVED] スタブ運用を解消・Agent 起動時の処理コスト削減
  - 4 ファイル（_RUNBOOK / handoff_latest / index / log）+ 3 ディレクトリ（topics / ideas / insights）配下を archived/casual/ へ移動
...
| Vault ディレクトリ | `wiki/casual/` | `wiki/personal/`（その後 `wiki/archived/casual/` へ完全アーカイブ・2026-04-29） |
| pending ディレクトリ | `pending/casual/` | `pending/personal/`（旧パスは [MOVED] スタブ 3 ファイル残存・処遇判断待ち） |
...
**v4 補強（15代目）**: Wiki-Rex も Origin 情報を参照する可能性がある（Personal_Brain 読み取り専用クエリ）。Wiki-Personal と同等の文脈限定原則の下で運用される。
...
**v4 拡張（15代目）**: ROADMAP Stage 2「統合読み出し期」のテスト運用として Wiki-Rex の読み取り専用クエリ例外を新設したが、これは 1:1 原則の "投入分業" は維持しつつ "読み取りクエリ" のみ拡張する慎重な設計。
...
ADR-Role / ADR-NLM はいずれも 2026-04-27 制定で、本起票は 2026-04-28（翌日）。supersede 規則を厳守する形で v2 を新規制定し v1 を archived へ移動する。さらに 2026-04-28 中に v2→v3→v4 と同日で 2 度 supersede（同日複数 supersede のためバージョン suffix 規則を v4 §10 で確立）。
...
**v6.9 補足**: 2026-04-29 のボス手動 `git mv wiki/casual/* wiki/archived/casual/*` では Git 履歴が継承された（`git log --follow` で旧パス時代の履歴も追跡可能）。GitHub MCP の制約はあくまで AI ロールが書込する場合の話であり、ボス手動操作なら git の rename detection が機能する。
...
- **同日複数 supersede のためのバージョン suffix 規則**（v4 §10 で確立・例: `archived/ADR-Role-2026-04-28-v3.md`）

### 論点 5: wiki/casual/ 凍結フォルダーの処遇（v6.9 で確定）

ボス指摘「wiki/直下に凍結フォルダーを残すのは Agent の無駄な処理が増えるだけ」を受けて再評価:

- 当初の [MOVED] スタブ案（v6.7-v6.8）は「リンク切れ防止」を過大評価していた
- 実際の外部リンクは ADR archived 内（歴史記録・404 でも実害なし）と pending/casual/ の旧 flag のみ
- ボス手動 git mv による完全アーカイブ化が:
  - Agent 起動時の処理コスト削減（α 原則）
  - 後任の混乱解消（archived/ は意味的に「凍結」と即理解可能）
  - 履歴追跡性向上（ファイル本体が archived/ に残る）
  - ADR-Vault 原則の射程外（ボス手動操作・AI ロールは触らない）

→ **2026-04-29 ボス手動で `wiki/casual/` を `wiki/archived/casual/` へ完全アーカイブ化（Phase Casual-Final-Archive 完了）**
...
| ADR | 旧版 | 新版 | 主な変更 |
|---|---|---|---|
| ADR-Role | v1（2026-04-27 制定）| v2 → **v3** → **v4（最終）** | 5 ロール表 → 6 ロール体制（Wiki-Rex 追加）/ §0 二系統管轄 / §12 STARTUP_CODES 訂正 / §14 構造変更境界 / §15 ADR 通知伝達経路 / §16 Wiki-Rex / §17 読み取り専用クエリ権限カテゴリ |
| ADR-NLM | v1（2026-04-27 制定）| v2（2026-04-28）| §2 担当マトリクスの NLM 名更新、§5 Casual → 専門 NLM 知見昇格ルールを Personal → 専門に更新、§ 改名 Note 追記 |

旧版は `wiki/adr/archived/` に日付付き or バージョン suffix 付きで保管。

### registry 同期

- `registry/roles.md`：ロール名・ロール正式名更新・Wiki-Rex 追加・Wiki-Personal 改名 Note に最終アーカイブ完了追記（v6.9）
- `registry/nlm.md`：NLM 名・担当ロール更新・Wiki-Rex 読み取り専用クエリ例外反映（v6.9）

### 運用文書改訂

- `wiki/STARTUP_CODES.md` v3 → v4 → v5（Wiki-Rex 追加）
- `CLAUDE.md` v1.2 → v1.3 → v1.4（Wiki-Rex 反映・4ロール明示）

### Vault 物理改名（Step 4 で実施・2026-04-28）+ 完全アーカイブ（Phase Casual-Final-Archive・2026-04-29）

- `wiki/casual/` → `wiki/personal/`（2026-04-28・15 代目）
- サブ層 5 層新設 + README（2026-04-28・15 代目）
- 既存ファイル移設（2026-04-28・15 代目）
- `wiki/casual/` → `wiki/archived/casual/` 完全アーカイブ（2026-04-29・ボス手動 git mv）
- `pending/casual/` → `pending/personal/` 移行（2026-04-28・[MOVED] スタブ 3 ファイル残存）
- `_RUNBOOK.md` v3 起草・`handoff_latest.md` 改名反映等は **Personal-Planner 業務として次スレで実施**

### NotebookLM 操作（ボス手動・Step 5）

- 表示名を `REX_Casual_Brain` → `REX_Personal_Brain` に変更（完了）
- UUID 不変のため `notebooklm-mcp-cli` 設定の更新不要

### handoff/latest.md（段階改訂）

- v6.4 → v6.5（13代目）→ v6.6（14代目）→ v6.7（15代目 Phase Personal-Migration）→ v6.8（15代目 Phase Wiki-Rex-Init）→ v6.9（15代目 Phase Casual-Final-Archive）
...
- 2026-04-28 15 代目統括 Evaluator: ADR-Role v3 supersede（§0 二系統管轄明文化・§12 STARTUP_CODES 訂正・§14 構造変更境界・§15 ADR 通知伝達経路）→ Phase Personal-Migration Step 1〜4 物理実施 → ステータス追記
- 2026-04-28 15 代目統括 Evaluator (続): Phase Wiki-Rex-Init 実施（ADR-Role v3 → v4 supersede / Wiki-Rex 新設 / 読み取り専用クエリ権限カテゴリ新設 / STARTUP_CODES v5 / CLAUDE.md v1.4 / latest.md v6.8）
- **2026-04-29 15 代目統括 Evaluator**: Phase Casual-Final-Archive 実施（ボス手動 git mv で wiki/casual/ → wiki/archived/casual/ 完全アーカイブ → registry / pending/INDEX / latest.md v6.9 で参照訂正完了）
...
```
Step 3: ADR 昇格（Wiki-Eval 専属）                          ✅ 14代目完了
Step 3': ADR-Role v2 → v3 supersede                       ✅ 15代目完了（commit aecf7f1）
Step 3'': ADR-Role v3 → v4 supersede                      ✅ 15代目完了（commit 19943cc・Phase Wiki-Rex-Init）
Step 4: 物理改名作業（Wiki-Eval 起動）                       ✅ 15代目完了
  - wiki/casual/ → wiki/personal/ ファイル単位移行
  - サブ層 5 層新設 + README 設置
  - 既存ファイル移設
  ⚠️ 中身改訂（_RUNBOOK v3 / handoff 改名 / philosophy → shuhari 等）は Personal-Planner 業務

Step 4': STARTUP_CODES.md v4 / CLAUDE.md v1.3 改訂          ✅ 15代目完了（commit 1262090）
Step 4'': STARTUP_CODES.md v5 / CLAUDE.md v1.4 改訂         ✅ 15代目完了（Phase Wiki-Rex-Init）
...
*更新: 15 代目統括 Evaluator (Opus 4.7) / 2026-04-28（ステータス追記・ADR-Role v3 supersede 反映）→ 2026-04-28（Phase Wiki-Rex-Init 反映）→ 2026-04-29（Phase Casual-Final-Archive 反映）*
*関連: ADR-Role v1→v2→v3→v4 / ADR-NLM v1→v2 / registry/roles.md / registry/nlm.md / STARTUP_CODES.md v3→v5 / CLAUDE.md v1.2→v1.4 / wiki/casual/ → wiki/personal/ → wiki/archived/casual/ 段階移行完了 / ROADMAP.md*
