...
バグを生成させないためにif then的発想で細かく規制すると反対に雑音となりAIの自律的創造性を反対に阻害していまると感じたんだ。

各_code.mdのQ&Aに関しては先代EvaluatorおよびAdviser達の気づきであって、それを選択するかは新任者が課題に直面したときに１次資料として取り入れるか判断すればいい。
Obisidianを導入した主な目的は、プロジェクト進捗とロジック設計の微細な変更を各担当者がリアルタイムで共有できるところだ。NLMのプロジェクト専用ラグとは違い敢えて単一の大脳皮質として設計したのも自律的判断と創造性を確保する目的もある。
