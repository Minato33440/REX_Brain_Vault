...
|---|---|---|---|---|---|
| [ADR-Role](ADR-Role.md) | Roles and Permissions | Accepted | 2026-04-28 | **v3** | [registry/roles.md](../registry/roles.md) |
...
> **ADR 本体は常に最新版を指す固定パス**（日付・バージョンをファイル名に含めない）。詳細は ADR-Role v3 §10 参照。
...
| [ADR-Role v2](archived/ADR-Role-2026-04-28.md) | [ADR-Role v3](ADR-Role.md) | **2026-04-28** | **統括 Evaluator 二系統管轄明文化（§0 新設）・STARTUP_CODES.md 管轄訂正（Personal-Planner → Wiki-Eval, §12）・構造変更 vs 中身変更の境界線新設（§14）・ADR 通知伝達経路新設（§15）** |
...
```
ADR-Role v3 (基盤: 全ADRの権限定義 + 1:1 NLM原則 + 二系統管轄)
  ├── ADR-Repo (物理リポ構成)
...
- 詳細は ADR-Role v3 §10「ADR本体の固定パス原則」参照
